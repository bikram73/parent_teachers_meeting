...
if ($_SESSION['user_role'] === 'parent') {
    ...
    // For teachers, only show students in their subject
    if ($_SESSION['user_role'] === 'teacher') {
        // First, try to get the subject from the session
        $teacher_subject = isset($_SESSION['subject']) ? $conn->real_escape_string($_SESSION['subject']) : null;
        
        // If subject is not set, fetch it from the teacher_users table
        if (!$teacher_subject) {
            $teacher_id = $conn->real_escape_string($_SESSION['user_id']);
            $subject_query = "SELECT subject FROM teacher_users WHERE id = ?";
            ...
            if ($subject_result && $subject_result->num_rows > 0) {
                $teacher_row = $subject_result->fetch_assoc();
                $teacher_subject = $conn->real_escape_string($teacher_row['subject']);
                
                // Update session with the subject for future use
                $_SESSION['subject'] = $teacher_subject;
            }
        }
        
        // If we have a subject, filter records; otherwise, show an empty result set
        if ($teacher_subject) {
            $sql = "SELECT * FROM student_performance WHERE subject = ? ORDER BY usn ASC";
            $stmt = $conn->prepare($sql);
...
<body class="dashboard-page">
    ...
    <div class="container mt-4">
        <div class="page-content">
            ...
            <?php if($_SESSION['user_role'] === 'teacher' && $result->num_rows === 0): ?>
            <div class="alert alert-warning">
                No performance records found for your subject. 
                <?php if (!isset($_SESSION['subject'])): ?>
                    Please update your profile to include your teaching subject.
                <?php endif; ?>
            </div>
            <?php endif; ?>
